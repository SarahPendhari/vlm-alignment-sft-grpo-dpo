...
def main() -> None:
    ...
    global_step = 0
    opt_step = 0
    debug_print = env_int("GRPO_DEBUG_PRINT", 0)
    debug_print_first_n = env_int("GRPO_DEBUG_PRINT_FIRST_N", 3)
    ...
    for ex in islice(local_ds, cfg.smoke_n):
        ...
        if debug_print and global_step < debug_print_first_n:
            print("\n" + "=" * 80)
            print(f"[debug] step={global_step} question_id={ex.get('question_id', None)}")
            print(f"[debug] question: {question}")
            print(f"[debug] gt_answers (first 10): {[a for a in answers[:10]]}")
            for j, gen in enumerate(raw_generations):
                parsed = extract_answer(gen)
                print("-" * 80)
                print(f"[debug] gen[{j}] raw:   {gen!r}")
                print(f"[debug] gen[{j}] parsed: {parsed!r}")
                print(f"[debug] gen[{j}] reward: {float(rewards[j].item()):.4f}")
            print("=" * 80 + "\n")
